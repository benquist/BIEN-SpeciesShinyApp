...
2026-05-10 | "Lets go ahead and make these changes. Also remember to update the project log for items we have learned during the history of app development" — Implemented all six coder-proposed fixes for the Markhamia lutea conservative-profile failure mode (see KNOWN_ISSUES_AND_LESSONS.md Issue 14). Changes to app.R: (1) Renamed sidebar checkbox "Conservative default profile" → "Strict-only BIEN profile (no auto-relaxation)" with rewritten tooltip documenting NSR coverage caveat. (2) Flipped default value TRUE → FALSE so granular filter toggles are visible by default. (3) Restricted query_occurrence_with_fallback plan ladder to plans[1] only when use_default_profile is TRUE — strict or empty, never silent fallback. (4) Added persistent yellow banner output$occ_strategy_banner_ui above leafletOutput("occurrence_map") that fires whenever effective occ_strategy != "strict", naming the strategy and dropped constraints. (5) Added bien_query_strategy column to every returned occurrence row in both return paths (plan-loop return and best_nonempty_result fallback return). (6) Added opt-in conditionalPanel checkbox "Strict native (exclude unevaluated)" under natives_only that threads strict_native_no_unknown=TRUE through query_occurrence_randomized → natives_check_with_null_fallback, emitting SQL "AND is_introduced=0" with no NULL fallback for Old-World taxa. Updated KNOWN_ISSUES_AND_LESSONS.md with full Issue 14 entry (symptom, diagnosis, three root causes, six-part fix, six lessons learned including NSR coverage scope, per-row provenance, persistent banners > toasts, Old-World canary species, is_cultivated NULL trap). Parse verified clean (Rscript -e "parse('app.R')" → 61 top-level expressions). File: BIEN-SpeciesShinyApp/app.R, BIEN-SpeciesShinyApp/KNOWN_ISSUES_AND_LESSONS.md.

38. Date: 2026-08-05
Prompt: Coordinate a redesign of the BIEN Species Shiny app front page: make the BIEN icon link to the BIEN website, reduce top dead space and bring the occurrence map forward without removing information or search behavior, and correct Americas-focused statements to reflect BIEN 4.2 global scope. Preserve the searched-species photo.
Source session: current workspace session
Outcome: Coordinated design-atelier, scandinavian-design, coder, ecology-user, and biodiversity-informatics-checker reviews. Updated app.R with an accessible linked BIEN logo, compact header and species-photo presentation, responsive visible evidence/flag panels, a viewport-aware occurrence map, and revised BIEN 4.2 global-scope language with uneven-coverage caveats. The searched-species photo remains in the desktop/tablet header; the existing narrow-mobile hide behavior remains. Validation: app.R parse and stale-copy checks passed; all project regression tests passed; Playwright confirmed the requested logo URL, no desktop/mobile horizontal overflow, non-overlapping evidence panels, and responsive map heights.

2026-08-05 | "Lets go ahead and implement these changes" (implementing the prior Scandinavian-minimal Occurrence-page proposal). Outcome: Implemented the four proposed changes in BIEN-SpeciesShinyApp/app.R without removing any control, disclosure, tab, or caveat: (1) Wrapped the header BIEN logo <img> in an accessible <a href="https://bien.nceas.ucsb.edu/bien/" target="_blank" rel="noopener noreferrer" aria-label="Visit the BIEN project website (opens in a new tab)"> with hover/focus-visible affordance CSS. Note: a concurrent uncommitted edit in this shared workspace had already added an equivalent logo-link wrapper and .bien-logo-link CSS rule at the time of this edit; the two implementations were reconciled into a single non-duplicated <a> (removed an accidental nested double-anchor and a duplicate CSS block) rather than layering two links. (2) Grouped the Occurrence tab's status banners (onboarding, strategy, evidence-quality/flag-composition grid) into a `.bien-status-stack` wrapper above the map, and the post-map banners (map caption, copy-R-code, overview notice, slow-query alert, summary-warn rail) into a second `.bien-status-stack`, replacing ad hoc `br()` gaps with one consistent 10px rhythm via `.bien-status-stack > *`. (3) Added a `@media (min-width: 1400px)` rule widening the main/sidebar Bootstrap columns (18%/82%) so the occurrence map claims more width on large desktop viewports; existing 3/9 split is unchanged below that breakpoint. (4) Added a `@media (max-width: 767px)` rule clamping `#occurrence_map` to 420px so the map does not dominate the initial mobile scroll, consistent with the app's existing 900px/640px breakpoints. Validation: `Rscript -e "parse('app.R')"` passed; `shinyApp` object instantiated without error; app served locally (200 OK) and rendered HTML was inspected to confirm a single non-duplicated logo `<a>`, the `.bien-status-stack` wrappers, and the map's height attribute — then the local test server was stopped. Not committed/pushed: `git status` showed this file already carried other uncommitted edits from a concurrent session in this shared environment (see the same-day prompt_log.md entry describing a related but separately authored redesign); committing here would mix two unrelated authors' changes without confirmation, so this turn's edits were left staged for the user to review and commit together. File: BIEN-SpeciesShinyApp/app.R.

39. Date: 2026-08-05
Prompt: Please update the Shiny app with the new suggested changes and push the changes.
Source session: current workspace session
Outcome: Revalidated the combined app worktree with a clean R parse, the complete regression suite, and a stale BIEN geographic-scope phrase scan. Deployed shinyapps.io bundle 12371526 to https://benquist.shinyapps.io/bien-species-shinyapp/. Live Playwright verification confirmed the BIEN logo links to the requested project URL, the searched-species photograph remains visible with attribution on desktop, the occurrence map renders at the responsive height, evidence panels do not overlap, no horizontal overflow occurs, and the BIEN 4.2 global-scope caption is present. The app source, regression tests, package artifact, deployment metadata, and project provenance were committed and pushed together.
