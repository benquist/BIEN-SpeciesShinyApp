...
server <- function(input, output, session) {
  ...
  output$bien_query_code <- renderText({
    ...
    paste(
      ...
      ")",
      "",
      "# Optional species-level join (many-to-many; keep originals for analyses)",
      "occ_with_traits <- occ_all %>%",
      "  left_join(traits_all, by = \"scrubbed_species_binomial\")",
      "",
      "# 3) Helper functions modeled after the Shiny app summaries",
      "find_first_col <- function(df, candidates) {",
      ...
      "",
      "summarize_observation_sources <- function(df) {",
      "  if (!is.data.frame(df) || nrow(df) == 0) return(data.frame(message = 'No observation records available.'))",
      "  df <- categorize_observation_records(df)",
      "  obs_type_col <- find_first_col(df, c('observation_type', 'observation.type'))",
      "  source_col <- find_first_col(df, c('datasource', 'data_source', 'collection', 'source'))",
      "  dataset_col <- find_first_col(df, c('dataset', 'dataset_name'))",
      ...
      "  x <- trimws(as.character(x))",
      "  x[x == ''] <- NA_character_",
      "  token_list <- stringr::str_extract_all(x, '-?[0-9]*\\\\.?[0-9]+(?:[eE][+-]?[0-9]+)?')",
      "  token_n <- lengths(token_list)",
      "  lower_x <- tolower(ifelse(is.na(x), '', x))",
      "  ambiguous_value <- stringr::str_detect(lower_x, '[0-9][[:space:]]*[-–/][[:space:]]*[0-9]') |",
      "    stringr::str_detect(lower_x, '\\\\bto\\\\b|×| x | by ')",
      ...
      "",
      "prepare_trait_visual_data <- function(traits) {",
      "  if (!is.data.frame(traits) || nrow(traits) == 0) return(NULL)",
      "  trait_name_col <- find_first_col(traits, c('trait_name', 'trait'))",
      "  trait_value_col <- find_first_col(traits, c('trait_value', 'value'))",
      "  unit_col <- find_first_col(traits, c('unit', 'units'))",
      "  if (is.null(trait_name_col) || is.null(trait_value_col)) return(NULL)",
      "",
      "  plot_df <- traits %>%",
      "    mutate(",
      "      trait_name_std = as.character(.data[[trait_name_col]]),",
      ...
      "      df <- .x",
      "      num_vals <- df$trait_value_num[!is.na(df$trait_value_num)]",
      "      n_numeric_used <- length(num_vals)",
      "      is_continuous <- n_numeric_used >= max(3, ceiling(0.6 * nrow(df))) && length(unique(num_vals)) > 1",
      "",
      "      if (is_continuous) {",
      "        tibble(",
      ...
      "          n_records = nrow(df),",
      "          n_numeric_used = n_numeric_used,",
      "          mean_value = mean(num_vals),",
      "          min_value = min(num_vals),",
      "          max_value = max(num_vals),",
      "          modal_value = NA_character_",
      "        )",
      "      } else {",
      "        val_tbl <- sort(table(df$trait_value_std), decreasing = TRUE)",
      ...
      "}",
      "",
      "# 4) App-like occurrence summary statistics",
      "occ_for_summary <- categorize_observation_records(occ_all)",
      "lat_col <- find_first_col(occ_for_summary, c('latitude', 'lat', 'decimalLatitude', 'decimal_latitude'))",
      "lon_col <- find_first_col(occ_for_summary, c('longitude', 'lon', 'decimalLongitude', 'decimal_longitude'))",
      "",
      "has_coords <- !is.null(lat_col) && !is.null(lon_col)",
      "if (has_coords) {",
      "  occ_for_summary <- occ_for_summary %>%",
      "    mutate(",
      "      lat_num = suppressWarnings(as.numeric(.data[[lat_col]])),",
      "      lon_num = suppressWarnings(as.numeric(.data[[lon_col]])),",
      "      coord_valid = !is.na(lat_num) & !is.na(lon_num) & lat_num >= -90 & lat_num <= 90 & lon_num >= -180 & lon_num <= 180",
      "    )",
      "  occ_mappable <- occ_for_summary %>% filter(coord_valid)",
      "} else {",
      "  occ_for_summary$coord_valid <- FALSE",
      "  occ_mappable <- occ_for_summary[0, , drop = FALSE]",
      "}",
      "",
      "occ_summary <- list(",
      "  species = species_name,",
      "  occurrence_records_returned = nrow(occ_for_summary),",
      "  mappable_points = nrow(occ_mappable),",
      "  observation_category_counts = sort(table(occ_for_summary$observation_category), decreasing = TRUE),",
      "  introduced_status_counts = if ('native_status' %in% names(occ_for_summary)) sort(table(tolower(as.character(occ_for_summary$native_status))), decreasing = TRUE) else 'native_status column not returned',",
      "  cultivated_status_counts = if ('is_cultivated' %in% names(occ_for_summary)) sort(table(tolower(as.character(occ_for_summary$is_cultivated))), decreasing = TRUE) else 'is_cultivated column not returned',",
      "  source_mix = summarize_observation_sources(occ_for_summary)",
      ")",
      "",
      "print(occ_summary$species)",
      "print(occ_summary$occurrence_records_returned)",
      "print(occ_summary$mappable_points)",
      ...
      "lat_col_lib <- find_first_col(occ_liberal_summary, c('latitude', 'lat', 'decimalLatitude', 'decimal_latitude'))",
      "lon_col_lib <- find_first_col(occ_liberal_summary, c('longitude', 'lon', 'decimalLongitude', 'decimal_longitude'))",
      "if (!is.null(lat_col_lib) && !is.null(lon_col_lib)) {",
      "  occ_liberal_map <- occ_liberal_summary %>%",
      "    mutate(",
      "      lat_num = suppressWarnings(as.numeric(.data[[lat_col_lib]])),",
      "      lon_num = suppressWarnings(as.numeric(.data[[lon_col_lib]])),",
      "      coord_valid = !is.na(lat_num) & !is.na(lon_num) & lat_num >= -90 & lat_num <= 90 & lon_num >= -180 & lon_num <= 180",
      "    ) %>% filter(coord_valid)",
      "} else {",
      "  occ_liberal_map <- occ_liberal_summary[0, , drop = FALSE]",
      "}",
      ...
      "",
      "if (!is.null(trait_vis)) {",
      "  trait_summary_tbl <- trait_vis$summary",
      "  print(head(trait_summary_tbl, 20))",
      "",
      "  cont_traits <- trait_summary_tbl %>%",
      "    filter(value_type == 'continuous') %>%",
      "    slice_head(n = 6)",
      "",
      "  if (nrow(cont_traits) > 0) {",
      "    plot_df <- trait_vis$data %>%",
      "      semi_join(cont_traits, by = c('trait_name_std', 'unit_std')) %>%",
      "      filter(!is.na(trait_value_num))",
      "",
      "    ggplot(plot_df, aes(x = trait_value_num)) +",
      "      geom_histogram(bins = 30, fill = '#66c2a5', color = 'white') +",
      "      facet_wrap(~ paste0(trait_name_std, ' [', unit_std, ']'), scales = 'free', ncol = 2) +",
      "      labs(",
      "        title = paste('Trait distributions for', species_name),",
      "        x = 'Trait value',",
      "        y = 'Record count'",
      "      ) +",
      "      theme_minimal(base_size = 12)",
      "  } else {",
      "    message('No continuous trait variables available to plot; see categorical summary table instead.')",
      "  }",
      "} else {",
      "  message('No trait records available for summary/plotting.')",
      "}",
      "",
